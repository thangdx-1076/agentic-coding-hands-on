...
/**
 * Sun* logo, multi-color (code-rules §2a "multi-color icon" exception — kept
 * as-is, not converted to `currentColor`). Extracted from the inline SVG
 * that used to live directly in `(home)/_components/widget-button.tsx`
 * (`186:1766;214:3762`, the pill's logo) so the "Thể lệ" option button
 * (`214:3752`, FAB expanded panel `Sv7DFwBw1h`) can reuse the exact same
 * path/gradient data instead of a second copy (DRY,
 * `clarifications.md` § DRY — không tạo asset/icon mới).
 *
 * Default `width="20" height="19"` matches the pill's original render size
 * exactly (nothing renders wider/narrower after this extraction). Gradient
 * ids (`paint0_linear_14788_4887`/`paint1_...`) are static: the pill and the
 * "Thể lệ" option never render at the same time (open state unmounts the
 * pill content), so only one instance of this component ever exists in the
 * DOM at once — if that invariant changes, switch to `useId()`.
 */
export function IconSunLogo(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      width="20"
      height="19"
