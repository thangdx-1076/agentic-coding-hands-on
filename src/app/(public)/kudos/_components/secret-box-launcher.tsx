"use client";

import { useState } from "react";

import { openSecretBoxAction } from "../_actions/open-secret-box";
import { useSecretBoxDialog } from "../_hooks/use-secret-box-dialog";
import type { BadgeKey } from "../_utils/secret-box-badge-asset";

import type { KudosStats, KudosStatListCopy } from "./kudos-stat-list";
import { SecretBoxDialog } from "./secret-box-dialog";

export type SecretBoxLauncherProps = {
  stats: KudosStats;
  copy: KudosStatListCopy;
};

const DEFAULT_OPEN_GIFT_DISABLED_TITLE = "Tính năng đang được phát triển";

/** mm:I2940:13497;186:1766 `MM_MEDIA_Open Gift` — inlined + `currentColor`
 * (code-rules 2a) so it inherits the button's `text-login-button-text`.
 * Moved here from `kudos-stat-list.tsx` (phase 05) — the trigger button now
 * lives in this composition root, not in the presentational stat list. */
function IconOpenGift(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden="true"
      {...props}
    >
      <path
        d="M22.5 10.3698L19.76 8.77984C20 8.56984 20.23 8.29984 20.4 7.99984C21.23 6.56984 20.74 4.72984 19.3 3.89984C18.44 3.39984 17.43 3.39984 16.58 3.75984L16.59 3.74984L15.71 4.13984L15.6 3.17984L15.59 3.18984C15.5 2.27984 14.97 1.39984 14.11 0.899841C12.67 0.0748415 10.84 0.569842 10 1.99984C9.83 2.29984 9.72 2.62984 9.66 2.94984L6.91 1.36984C5.95 0.819842 4.73 1.13984 4.18 2.09984L2.68 4.69984C2.4 5.17984 2.57 5.78984 3.05 6.05984L4.78 7.05984L9 9.49984H2.5V19.4998C2.5 20.6098 3.4 21.4998 4.5 21.4998H20.5C21.61 21.4998 22.5 20.6098 22.5 19.4998V14.3698L23.23 13.0998C23.78 12.1398 23.46 10.9198 22.5 10.3698ZM16.94 5.99984C17.21 5.49984 17.83 5.35984 18.3 5.62984C18.78 5.90984 18.95 6.49984 18.67 6.99984C18.39 7.49984 17.78 7.63984 17.3 7.36984C16.83 7.08984 16.66 6.49984 16.94 5.99984ZM14.57 8.09984L21.5 12.0998L20.5 13.8298L13.57 9.82984L14.57 8.09984ZM11.5 19.4998H4.5V11.4998H11.5V19.4998ZM11.84 8.82984L4.91 4.82984L5.91 3.09984L12.84 7.09984L11.84 8.82984ZM12.11 4.36984C11.63 4.08984 11.47 3.49984 11.74 2.99984C12 2.49984 12.63 2.35984 13.11 2.62984C13.59 2.90984 13.75 3.49984 13.47 3.99984C13.2 4.49984 12.59 4.63984 12.11 4.36984ZM13.5 19.4998V12.0998L20.5 16.1398V19.4998H13.5Z"
        fill="currentColor"
      />
    </svg>
  );
}

/**
 * Composition root for the Secret Box flow (SM-001) — the one seam between
 * Track A (`SecretBoxDialog`, phase 03) and Track B (`openSecretBoxAction`,
 * phase 04). Owns the trigger button, the dialog's imperative lifecycle
 * (`useSecretBoxDialog`), and the 3-state machine (Closed/Unopened/
 * Revealed). Imports the `"use server"` action directly — same precedent as
 * `logout.ts`/`set-locale.ts` — so no new prop threads through
 * `kudos-client.tsx`/`kudos-screen.tsx`/`kudos-sidebar.tsx` (D-P03).
 *
 * `canMount` is fixed from `stats.secretBoxUnopened` — the SERVER-rendered
 * count at first paint — and never recomputed from `live`, the state this
 * component updates after every open. A mounted-but-closed native
 * `<dialog>` still counts as 1 DOM element (`toHaveCount(0)`, S13), so at
 * 0 unopened the dialog must never mount at all; conversely, using `live`
 * instead of the server count would unmount the dialog the instant the
 * last box is opened (live hits 0 mid-interaction), breaking the open
 * dialog out from under the user (S07/S09/S10). Once mounted, it stays
 * mounted for the rest of this page's lifetime — closing only hides it via
 * `useSecretBoxDialog.close()` (S11/S12).
 */
export function SecretBoxLauncher({ stats, copy }: SecretBoxLauncherProps) {
  const { open, close, registerDialog, onCancel } = useSecretBoxDialog();
  const [live, setLive] = useState(stats.secretBoxUnopened);
  const [badgeKey, setBadgeKey] = useState<BadgeKey | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const canMount = stats.secretBoxUnopened > 0;

  async function handleOpenBox() {
    if (busy) {
      return;
    }
    setBusy(true);
    setError(null);
    try {
      const result = await openSecretBoxAction();
      if (result.ok) {
        // Server-authoritative values only (FR-601) — never derived by
        // decrementing `live` locally.
        setBadgeKey(result.badgeKey);
        setLive(result.unopened);
      } else {
        setError(copy.secretBox.error);
      }
    } finally {
      setBusy(false);
    }
  }

  return (
    <>
      {/* mm:2940:13497 */}
      <button
        type="button"
        disabled={!canMount}
        title={
          canMount
            ? undefined
            : (copy.openGiftDisabledTitle ?? DEFAULT_OPEN_GIFT_DISABLED_TITLE)
        }
        data-testid="kudos-open-gift"
        onClick={open}
        className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg bg-login-button px-4 py-4 font-montserrat text-[22px] leading-7 font-bold text-login-button-text"
      >
        <IconOpenGift className="h-6 w-6" />
        {copy.openGift}
      </button>

      {canMount && (
        <SecretBoxDialog
          registerDialog={registerDialog}
          copy={copy.secretBox}
          state={badgeKey ? "revealed" : "unopened"}
          unopenedCount={live}
          awardedBadgeKey={badgeKey}
          canOpen={live > 0 && !busy}
          busy={busy}
          onOpenBox={() => {
            void handleOpenBox();
          }}
          onClose={close}
          onCancel={onCancel}
        />
      )}

      {/* `SecretBoxDialog` (phase 03, locked) has no `error` prop — a failed
          `openSecretBoxAction` surfaces here instead, outside the dialog,
          rather than fabricating a badge (FR-601, functional-spec § 9). */}
      {error && (
        <p
          data-testid="secret-box-launch-error"
          role="alert"
          className="mt-2 text-center font-montserrat text-[13px] font-bold text-red-400"
        >
          {error}
        </p>
      )}
    </>
  );
}
