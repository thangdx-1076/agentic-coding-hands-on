...
## 14. Test Policy (Delivery pending — spec draft, chưa implement)

**Chính sách:** `e2e-red-first` (chốt tại `clarifications.md` — màn hình có route resolution,
dropdown, nhánh 404, chuyển tiếp trạng thái rỗng: hành vi, không phải thuần trình bày). Bằng chứng
RED/GREEN CHƯA có ở lượt spec draft này — đây là việc của phiên implement kế tiếp
(`tests/e2e/profile.spec.ts`, `pnpm test:e2e`). Xem technical-spec.md § 5.1 cho danh sách SC-###
mà tester sẽ dùng làm checklist khi implement.
