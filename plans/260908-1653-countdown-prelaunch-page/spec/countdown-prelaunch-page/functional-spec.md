...
- Khi đếm ngược đã về 0, khoá gỡ hoàn toàn dù cờ còn bật; vào `/prelaunch` lúc này bị đưa về `/`.
  (BR-003)
- **BR-005** — Mở `config.matcher` không được kéo theo Supabase overreach: `proxy()` kiểm tra khoá
  trước, và route ngoài whitelist cũ trả `NextResponse.next()` ngay, không gọi `getUserOrNull`.
