...
# CountdownPrelaunchPage — Technical Spec
...
|---|---|---|---|---|
| **DEC-001** | flow | `PRELAUNCH_LOCK_ENABLED === "true"` AND `!reached` AND pathname ∉ exempt list | redirect sang `/prelaunch`, giữ nguyên đích gốc không lộ ra URL | TBD (draft) |
...
Chưa có code mới (`status: draft`). Tái dùng nguyên trạng, chỉ đổi vị trí: `(home)/_utils/countdown.ts` → `src/utils/countdown.ts`; `(home)/_hooks/use-countdown.ts` → `src/hooks/use-countdown.ts`; `(home)/_components/countdown-tiles.tsx` → `src/components/countdown-tiles.tsx`; `src/proxy.ts` (mở rộng, không thay file). Mới, chưa viết: `src/app/(public)/prelaunch/page.tsx`, khoá i18n `prelaunch.*` trong `messages/{vi,en}.json`.

#### Data Flow

Không áp dụng — feature không có DB write nào; xem § 3 cho luồng Request → BE → Rule → Result của từng action.
