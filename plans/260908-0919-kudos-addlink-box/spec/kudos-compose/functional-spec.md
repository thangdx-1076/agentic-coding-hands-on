...
**Traceability:** F009_KudosCompose → SCR008_KudosCompose → US001, US002, US003, US004
...
(readonly, chưa có `onClick`). Riêng nút "Chèn liên kết" trong toolbar định dạng hiện dùng
`window.prompt` của trình duyệt — không kiểm soát được lỗi nhập liệu, không nhất quán về giao diện
với phần còn lại của dialog, và không có bằng chứng test tự động.
...
xong dialog đóng và Kudo mới hiện ngay trên bảng. Nút "Chèn liên kết" trên toolbar mở thêm một
dialog `<dialog>` native thứ hai — "Thêm đường dẫn" — lồng trên dialog Viết Kudo, cho nhập Nội dung
liên kết và URL rồi chèn markdown `[Nội dung](URL)` vào đúng vị trí đang soạn, thay cho
`window.prompt`.
...
ngay sau khi gửi; chèn liên kết qua dialog "Thêm đường dẫn" thay cho `window.prompt`.
...
rich-text editor thật (chỉ chèn ký hiệu markdown-subset vào textarea thường); dialog "Thêm đường
dẫn" không lưu draft, không responsive ngoài breakpoint desktop cố định (giống dialog cha).
...
|----|------------|------------------------|-----------------|---------------|-------------------|---------|
| CAP-01 | Soạn và gửi Kudo | Sunner chọn người nhận, đặt danh hiệu, viết lời cảm ơn kèm hashtag/ảnh/tuỳ chọn ẩn danh (kể cả chèn liên kết qua dialog "Thêm đường dẫn") rồi gửi | US001, US002, US003, US004 | FR-001, FR-002, FR-101, FR-102, FR-201, FR-202, FR-203, FR-204, FR-205, FR-206, FR-207, FR-208, FR-209, FR-401, FR-402, FR-403, FR-404, FR-405, FR-406, FR-601 | BR-001, BR-002, BR-003, BR-004, BR-005, BR-006, BR-007, BR-008, BR-009, DEC-001, DEC-002, DEC-003, SM-001, SM-002 | SCR008_KudosCompose |
...
- **FR-209** Nút "Chèn liên kết" trên toolbar định dạng mở một dialog "Thêm đường dẫn" lồng trên
  dialog Viết Kudo, gồm 2 trường Nội dung liên kết và URL cùng 2 nút Hủy/Lưu.
...
- **FR-404** Thêm ảnh thứ 6, hoặc chọn sai định dạng file (không phải `.jpg`/`.png`), bị chặn kèm
  thông báo lỗi.
- **FR-405** Bấm "Lưu" trong dialog "Thêm đường dẫn" khi Nội dung hoặc URL không hợp lệ thì hiện lỗi
  đúng dưới từng trường và dialog đó vẫn mở, không chèn markdown.
- **FR-406** Bấm "Hủy" hoặc nhấn Escape trong dialog "Thêm đường dẫn" thì chỉ dialog đó đóng lại,
  nội dung textarea Viết Kudo không đổi, dialog Viết Kudo vẫn mở nguyên trạng.
...
- Nội dung liên kết trong dialog "Thêm đường dẫn" bắt buộc, tối đa 100 ký tự sau khi `trim()`.
  (BR-007)
- URL trong dialog "Thêm đường dẫn" bắt buộc, từ 5 đến 2048 ký tự sau `trim()`, phải parse được và
  chỉ nhận scheme `http`/`https` — cùng whitelist với bộ hiển thị markdown Kudo. (BR-008)
- Lưu hợp lệ chèn `[Nội dung](URL)` vào đúng vị trí selection đã chụp lúc mở dialog (ghi đè vùng
  chọn nếu có); Nội dung được prefill từ text đang bôi đen khi bấm nút Link, rỗng nếu không có
  selection. (BR-009)
...
- Thiếu bất kỳ trường bắt buộc nào lúc bấm Gửi thì báo lỗi đúng tại từng trường và không submit.
  (DEC-002)
- Bấm nút "Chèn liên kết" trên toolbar mở dialog "Thêm đường dẫn" lồng trên dialog Viết Kudo.
  (DEC-003)
- Trạng thái hiển thị của modal Viết Kudo đi qua 4 mốc: đóng, mở, đang gửi, đóng lại. (SM-001)
- Trạng thái hiển thị của dialog "Thêm đường dẫn" đi qua 4 mốc: đóng, mở, đang validate/lưu, đóng
  lại — độc lập với trạng thái của dialog Viết Kudo bên ngoài. (SM-002)

## 6. Screens

| Screen Name | SCR### | What User Sees | What User Can Do |
|-------------|--------|-----------------|-------------------|
| Viết Kudo (dialog) | SCR008_KudosCompose | Dialog che phủ `/kudos`, gồm tiêu đề, các trường Người nhận/Danh hiệu/Nội dung/Hashtag/Image/Checkbox ẩn danh, 2 nút Hủy/Gửi, và (lồng bên trong) dialog "Thêm đường dẫn" khi bấm nút Link | Chọn người nhận, đặt danh hiệu, viết nội dung có định dạng (kể cả chèn liên kết qua dialog "Thêm đường dẫn"), gắn hashtag/ảnh, chọn gửi ẩn danh, gửi hoặc huỷ |
...
3. Sunner chọn người nhận, đặt danh hiệu, viết nội dung, thêm hashtag (và tuỳ chọn ảnh/ẩn danh).
   Khi cần chèn liên kết, Sunner bấm nút "Chèn liên kết" trên toolbar — dialog "Thêm đường dẫn" mở
   lồng bên trên, Sunner nhập Nội dung + URL rồi bấm "Lưu" để chèn markdown vào đúng vị trí đang
   soạn, hoặc bấm "Hủy"/Escape để đóng dialog đó mà không đổi gì.
...
- [ ] Bấm nút "Chèn liên kết" trên toolbar, nhập Nội dung + URL hợp lệ rồi Lưu — markdown
      `[Nội dung](URL)` được chèn đúng vị trí đang soạn trong Nội dung.
...
### US001_SendKudo — Chèn liên kết (Happy Path)

**Given** dialog Viết Kudo đang mở và Sunner bấm nút "Chèn liên kết" trên toolbar, **When** nhập
Nội dung 1–100 ký tự và URL hợp lệ (`http`/`https`, 5–2048 ký tự) rồi bấm "Lưu", **Then** dialog
"Thêm đường dẫn" đóng và markdown `[Nội dung](URL)` được chèn vào Nội dung tại đúng vị trí (hoặc
thay vùng chọn) đã có lúc bấm nút "Chèn liên kết".

### US001_SendKudo — Error: liên kết không hợp lệ

**Given** dialog "Thêm đường dẫn" đang mở, **When** để trống hoặc nhập Nội dung/URL sai định dạng
rồi bấm "Lưu", **Then** lỗi hiển thị đúng dưới từng trường sai, dialog "Thêm đường dẫn" vẫn mở,
Nội dung của dialog Viết Kudo không đổi.
...
| Nội dung liên kết chỉ toàn khoảng trắng | bị coi là rỗng sau `trim()`, chặn Lưu | "Không được để trống." |
| URL liên kết chứa khoảng trắng | không parse được, chặn Lưu | "URL không hợp lệ — chỉ nhận http hoặc https." |
| URL liên kết dài đúng 2048 ký tự (biên trên) | vẫn hợp lệ, chèn markdown bình thường | "None — silent handling" |
| Đang bôi đen text trong Nội dung khi bấm "Chèn liên kết" rồi Lưu | vùng chọn bị thay bằng markdown mới, không giữ lại text cũ | "None — silent handling" |
| Nhấn Escape khi dialog "Thêm đường dẫn" đang mở | chỉ dialog đó đóng, dialog Viết Kudo và Nội dung giữ nguyên | "None — silent handling" |
...
- **FR-601** → Server Action luôn tự kiểm tra phiên đăng nhập, kể cả khi client bị qua mặt (gọi
  thẳng action).
- **FR-209** → Bấm nút "Chèn liên kết" luôn mở đúng dialog "Thêm đường dẫn" lồng trên dialog Viết
  Kudo, không còn lời gọi `window.prompt` nào trong `src/`.
- **FR-405** → Lưu với Nội dung hoặc URL không hợp lệ luôn hiện lỗi đúng trường và giữ dialog "Thêm
  đường dẫn" mở, không chèn markdown.
- **FR-406** → Hủy hoặc Escape trong dialog "Thêm đường dẫn" luôn chỉ đóng đúng dialog đó, không
  đổi Nội dung của dialog Viết Kudo.

## 11. Risks & Known Issues

| Risk | Detail | Mitigation / Status |
|------|--------|----------------------|
| `experimental.serverActions.bodySizeLimit` nâng lên `28mb` cho TOÀN BỘ app | Next 16 chỉ cho 1 giá trị app-wide, không có override theo action — mọi Server Action khác (`toggleKudoHeart`, `loadMoreKudos`, `logout`, `setLocale`...) cũng nhận giới hạn 28MB dù không cần | Đánh đổi có chủ đích (AD-4) — chỉ có phương án tốt hơn nếu chuyển sang upload trực tiếp từ browser (đổi kiến trúc lớn hơn); theo dõi, chưa cần fix ngay |
| Kudo ẩn danh do CHÍNH MÌNH gửi vẫn hiện nút tim ở trạng thái enabled | Client không còn cách nào biết kudo đó là của mình (sender bị ẩn qua view) — bấm vào thì bị RLS `kudo_hearts_insert_own` chặn ở DB, không phải bug mới | Known limitation, ghi rõ ở `plan.md` AD-2; UI không đổi vì không có tín hiệu client-side nào để phát hiện |
| Không giới hạn độ dài `content`/`title`/`anonymous_name` ở tầng DB | Cột `text` không CHECK constraint — giới hạn thực tế chỉ là `bodySizeLimit` 28MB | Quyết định có chủ đích (YAGNI) — spec không cho `maxLength`, dựng giới hạn tự phát minh sẽ sai với thiết kế |
...
| Supabase Storage | infrastructure | Bucket `kudo-images` — tạo bằng migration `0010_kudo_images_bucket.sql`, đã apply và verify trên instance local | FR-206, BR-003 |
