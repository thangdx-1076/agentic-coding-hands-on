...
lang: vi
---
<!-- REVISION draft — sửa spec khớp lại với MoMorph spec `hUyaaugye2` + code thật, KHÔNG phải
     tài liệu greenfield. So với bản `docs/vi/features/F002_LanguageSwitch/` hiện có (status:
     implemented), file này SỬA phần sai (§ 6 luôn hiện cờ VN) và THÊM phần thiếu (content-purity
     en.json, nhãn nav giữ tiếng Anh, chi tiết menu chọn ngôn ngữ theo MoMorph A/A.1/A.2). -->
...
| RISK-04 | known-issue | *(REVISION — mới)* Trigger luôn render `IconVnFlag` bất kể locale (`language-selector.tsx:62`, hardcode, không nhánh theo `label`) — vi phạm FR-201; chưa có component `IconEnFlag` nào trong `src/app/_components/language-selector/`. | Trigger sai cờ khi ở locale `en` — không khớp MoMorph spec A | confirmed |
| RISK-05 | known-issue | *(REVISION — mới)* Menu item (`language-selector.tsx:85-97`) chỉ render label chữ, không có icon cờ, không có style phân biệt cho item đang active, và không có kích thước cố định 110×56px (đang dùng `px-4 py-2` co giãn theo nội dung) — vi phạm FR-203. | Menu không khớp MoMorph spec A.1/A.2 | confirmed |
| RISK-06 | known-issue | *(REVISION — mới)* `messages/en.json` còn 7 leaf nguyên text tiếng Việt: `kudos.banner.title`, `kudos.compose.placeholder`, `kudos.heroSearch.placeholder`, `kudos.heroSearch.ariaLabel`, `kudos.spotlight.searchPlaceholder`, `kudos.feed.empty`, `kudos.sidebar.emptyBoard` — vi phạm FR-002. | Người dùng chọn `en` vẫn thấy tiếng Việt ở các khối Sun* Kudos này | confirmed |
...
| `IconEnFlag` (component mới, hiện chưa tồn tại) | component | Cần để trigger/menu hiển thị đúng cờ Anh theo FR-201/FR-203 | FR-201, FR-203 |
