...
lang: vi
---
<!-- REVISION draft — sửa/thêm 4 điểm cụ thể so với bản `docs/vi/features/F003_Homepage/` hiện có:
     (1) dòng mô tả thứ 3 của C1 còn thiếu, (2) nhãn nav "Awards Information" số nhiều thắng nhưng
     code ship số ít, (3) logo header 52×48 lệch spec 64×60, (4) font LED "Digital Numbers" chưa
     nạp — cần quyết định người. FAB pill 106×64 / inner group 42×32 GIỮ NGUYÊN — đã là accepted
     deviation có chữ ký ở clarifications.md, không đụng vào. -->
...
|------|----------|-------------------|-----------|--------------|
| D001 | Nội dung thật của menu widget hành động nhanh (spec không liệt kê option) | **RESOLVED 2026-09-08** — MoMorph cấp thiết kế thật cho component set `214:3916` (2 variant: đóng/mở). Nội dung: "Thể lệ" (`ROUTES.STANDARDS`) và "Viết KUDOS" (`ROUTES.KUDOS`). Trigger pill 106×64 morph thành nút tròn 56×56 đỏ `#D4271D` "×" khi mở. Chi tiết: `technical-spec.md` § 3.1 A6, § 4.3 SM-001. | Thiết kế thật đã có, không còn suy diễn | no |
...
- **FR-211** *(REVISION — mới, theo MoMorph row C1)* Khối tiêu đề mục Giải thưởng hiển thị ĐỦ 3
  dòng: caption nhỏ "Sun* annual awards 2025", tiêu đề lớn "Hệ thống giải thưởng", và dòng mô tả
  phụ "Các hạng mục sẽ được trao giải theo TOP những người xuất sắc nhất." — dòng thứ 3 hiện đang
  THIẾU, phải bổ sung vào cả `home-copy.ts` (vi) và `messages/en.json` (en).
- **FR-212** *(REVISION — sửa)* Link nav thứ 2 trong header hiển thị đúng nhãn SỐ NHIỀU "Awards
  Information" (KHÔNG phải "Award Information" số ít) — theo MoMorph row A1.3/7.3 và TC ID-21/23.
- **FR-213** *(REVISION — sửa)* Logo header có kích thước 64×60px (KHÔNG phải 52×48px như code
  hiện tại) — theo MoMorph row A1.1.
- **FR-214** *(REVISION — mới)* Chữ số trong mỗi ô đếm ngược (hero) dùng font 7-segment "Digital
  Numbers" (hoặc font thay thế đã được duyệt theo D003) — KHÔNG được rơi về `monospace` mặc định
  của trình duyệt.
...
**Business value:** Hạ tầng UI thông báo sẵn sàng cho tính năng notifications thật, không để
người dùng chờ một tính năng chưa tồn tại mà không có phản hồi gì.
...
đầy đủ hero/đếm ngược đang chạy (font LED)/thông tin sự kiện/khối Giải thưởng đủ 3 dòng/6 thẻ giải
thưởng/Sun* Kudos/footer, header có logo 64×60px và nhãn "Awards Information".
...
- **FR-214** → Tester xác nhận `font-family` của chữ số đếm ngược không phải `monospace` mặc định
  (phải là font LED thật hoặc thay thế đã duyệt theo D003).
...
| RISK-03 | known-issue | **ACCEPTED DEVIATION — không cần fix.** FAB pill 106×64 và inner group 42×32 lệch 1px so với design (`WidgetButton`) — đã được node data xác nhận là chấp nhận được tại `plans/260908-1103-home-widget-fab/clarifications.md:17,45-56`. | Không ảnh hưởng — giữ nguyên hiện trạng, KHÔNG đưa vào FR nào để "sửa" | accepted |
| RISK-04 | known-issue | *(REVISION — mới)* Khối tiêu đề Giải thưởng (C1) hiện chỉ render 2/3 dòng — thiếu dòng mô tả phụ "Các hạng mục sẽ được trao giải theo TOP những người xuất sắc nhất." — `AwardsSection` chỉ đọc `copy.awards.caption`/`copy.awards.heading` (`home-copy.ts:95-97`), type `awards` không có field mô tả thứ 3. Vi phạm FR-211. | Nội dung MoMorph không đủ | confirmed |
| RISK-05 | known-issue | *(REVISION — mới)* Nhãn nav ship "Award Information" số ít (`site-chrome.ts:69`) dù comment ngay bên cạnh (`site-chrome.ts:67-68`) ghi rõ spec/TC (số nhiều) mới là nội dung được chấp nhận — code tự mâu thuẫn với comment của chính nó. Vi phạm FR-212. | Nhãn hiển thị sai so với chính quyết định nội dung đã ghi trong code | confirmed |
| RISK-06 | known-issue | *(REVISION — mới)* Logo header 52×48px (`site-header.tsx:56-64`, `Image width={52} height={48}`) — spec A1.1 yêu cầu 64×60px. Vi phạm FR-213. | Logo nhỏ hơn thiết kế | confirmed |
| RISK-07 | known-issue | *(REVISION — mới, dùng chung với F011)* Font "Digital Numbers" cho digit đếm ngược chưa được nạp — không có file font nào dưới `public/`; `countdown-tiles.tsx:38-41` fallback `"Digital Numbers", monospace` → trình duyệt luôn render `monospace` vì font khai báo không tồn tại. Nợ đã ghi ở `plans/260908-1653-countdown-prelaunch-page/clarifications.md` § Ghi nợ nhưng KHÔNG đạt bar "UI chính xác tuyệt đối" — cần D003. Vi phạm FR-214. | Chữ số đếm ngược sai font thiết kế trên cả `/` và `/prelaunch` | confirmed |
