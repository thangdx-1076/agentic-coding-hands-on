...
---
<!--
FORWARD-DRAFT NOTICE (SecretBoxModal, plans/260908-1337-secret-box-modal):
Nội dung dưới đây là bản SAO NGUYÊN VĂN của `docs/vi/system/architecture.md` (đọc 2026-09-08),
cộng CHỈ phần delta mà SecretBoxModal giới thiệu. Mọi dòng gốc giữ nguyên 100% — không sửa,
không xoá. Phần MỚI nằm trọn trong mục cuối file. File này CHƯA merge vào
`docs/vi/system/architecture.md` thật; nó được promote ở implement-start và được đối chiếu lại
với as-built ở Delivery.
-->
...
> **[SecretBoxModal draft — chưa merge]** Delta của feature đang được xây trong
> `plans/260908-1337-secret-box-modal/`. Quyết định gốc: `clarifications.md § Session 260908`.
> KHÔNG cấp F### cho feature này ở đây — mã feature được cấp ở promote, không phải bây giờ.
...
`secretBoxOpened`/`secretBoxUnopened` (thay 2 giá trị hardcode `0` tại `page.tsx:146-147`), gộp
