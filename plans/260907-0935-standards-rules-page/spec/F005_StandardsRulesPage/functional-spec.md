...
- **FR-003** Giữ nguyên chính tả/ký tự đặc biệt của design: tên icon **"ROOT FUTHER"** (không tự
  sửa thành "FURTHER"), en-dash trong **"10–20"**, emoji **❤️** trong 2 đoạn văn (intro section 2,
  body section 3).
...
- **FR-103** Section 2 ("NGƯỜI GỬI KUDOS: SƯU TẬP TRỌN BỘ 6 ICON, NHẬN NGAY PHẦN QUÀ BÍ ẨN") hiển
  ...
  FLOW TO HORIZON/BEYOND THE BOUNDARY/ROOT FUTHER) + đoạn closing. Mỗi caption là text node thật
...
- [ ] Section 2 hiển thị đúng lưới 6 badge, giữ nguyên chính tả "ROOT FUTHER".
...
*(để trống — điền bởi `tester` sau khi implement, theo `e2e-red-first`: RED trước code, GREEN +
visual validation sau. `redTestFiles`/`redCommand`/`redExitCode`/`redFailure` do tester ghi lại
khi Track B bắt đầu.)*
