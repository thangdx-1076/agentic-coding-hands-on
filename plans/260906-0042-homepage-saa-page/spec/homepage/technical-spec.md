...
**Source:** `app/page.tsx` (planned) → `lib/auth/get-current-user-role.ts` (planned) → `components/home/home-copy.ts` (planned)

---

#### A2 · Đếm ngược thời gian thực (client hook)
— *(client tick, không HTTP)* → `` `useCountdown` ``
`FR-202` · `SCR-home` · `US001`

**Who** · Bất kỳ khách truy cập nào đang xem trang chủ
**FE** · `components/home/countdown-timer.tsx` (planned) gọi `useCountdown(targetIso, initialNowMs)` (planned, `hooks/use-countdown.ts`) — state đầu SEED từ prop server, không gọi `Date.now()` ở render đầu (khớp SSR); `setInterval` mỗi 60s cập nhật `nowMs`; lệch ≤1 phút SSR/hydrate được chấp nhận (`suppressHydrationWarning` — § 5.2).
**Request** · không có — input là 2 prop tĩnh từ A1 (`targetIso`, `initialNowMs`)
**BE** · không có handler BE riêng — phép tính thuần chạy trong `lib/event/countdown.ts` (planned) *(ALG-001 — § 4.5)*
**Rule**
...
**Result** · read-only — không ghi DB. Re-render 3 ô số (DAYS/HOURS/MINUTES) mỗi phút; không side effect khác.
**Source:** `hooks/use-countdown.ts` (planned) → `lib/event/countdown.ts` (planned)

---

#### A3 · Click logo/nav link đang active → cuộn lên đầu trang
— *(client, không HTTP)* → `` `NavLink` ``
`FR-102` · `SCR-home` · `US001`

**Who** · Bất kỳ khách truy cập nào, đang ở `/` và click logo hoặc link "About SAA 2025"
**FE** · `components/home/nav-link.tsx` (planned) — client leaf dùng `usePathname()` so khớp `href`; nếu đang active thì gắn `onClick` gọi `window.scrollTo({top:0, behavior:"smooth"})` thay vì để Next điều hướng lại
**Request** · không có
**BE** · không có
**Rule**
...
|---|---|---|---|---|
| **DEC-001** | flow | `pathname === href` (đã ở đúng trang) | Trang cuộn mượt lên đầu, không tải lại; nếu KHÔNG active thì Next điều hướng bình thường | `components/home/nav-link.tsx` (planned) |

**Result** · read-only — không ghi DB, không điều hướng khi active (chỉ scroll).
**Source:** `components/home/nav-link.tsx` (planned)
...
`FR-401` `FR-403` `FR-601` · `SCR-home` · `SM-001` · `US002`

**Who** · Người dùng đã đăng nhập (member hoặc admin)
**FE** · `components/home/account-menu.tsx` (planned) render `button[aria-haspopup="menu"]` + `[role="menu"]`; mở/đóng/roving-focus qua `useMenuKeyboardNav` (đã có, tái dùng từ F002) *(BR-006 — § 4.4)*. Item "Trang quản trị" chỉ render khi prop `role === "admin"` (nhận từ A1, không tự tính lại).
...
**Source:** `components/home/account-menu.tsx` (planned) → `hooks/use-menu-keyboard-nav.ts` (đã có) → `app/todo/actions.ts` (`logoutAction`, đã có, tái dùng)

---

#### A5 · Mở panel thông báo (bell)
— *(client render/open state)* → `` `NotificationBell` ``
`FR-402` · `SCR-home` · `US003`

**Who** · Người dùng đã đăng nhập (bell không render khi Anonymous — xem BR-001 ở A1)
**FE** · `components/home/notification-bell.tsx` (planned) render `button[aria-haspopup="dialog"]`; click mở `[role="dialog"]` chứa text "Bạn chưa có thông báo" (empty state, không fetch API vì chưa có bảng notifications — xem functional-spec.md § 12)
**Request** · không có
**BE** · không có
...
**Source:** `components/home/notification-bell.tsx` (planned)

---

#### A6 · Mở menu hành động nhanh (widget)
— *(client render/open state)* → `` `WidgetButton` + `useMenuKeyboardNav` ``
`FR-210` `FR-401` · `SCR-home` · `SM-001` · `US004`

**Who** · Bất kỳ khách truy cập nào (không phân biệt trạng thái đăng nhập)
**FE** · `components/home/widget-button.tsx` (planned) render pill cố định góc dưới phải (`button[aria-haspopup="menu"]`); mở `[role="menu"]` 2 `menuitem` ("Sun* Kudos" → `/kudos`, "Awards Information" → `/awards`) — nội dung 2 mục là `[INFERRED]` từ 2 icon thiết kế, chưa xác nhận với người dùng thật (xem functional-spec.md § 3 D001). Mở/đóng dùng chung `useMenuKeyboardNav` *(BR-006 — § 4.4)*.
**Request** · không có
**BE** · không có
...
**State** · `SM-001`: `closed` → `open` *(§ 4.3)*
**Source:** `components/home/widget-button.tsx` (planned) → `hooks/use-menu-keyboard-nav.ts` (đã có)
...
| A1 | Đọc role từ Supabase lỗi hoặc không có row `public.users` khớp `id` | `getCurrentAccount` fail-open trả `role: "member"` — Trang quản trị không hiện dù người dùng thật có thể là admin (xem BR-002) |
...
|---|---|---|---|
| `HomePage` (Server Component) | Render toàn bộ trang, đọc session+role, build props | A1 | `app/page.tsx` (planned) |
| `HomeHeader` | Header: logo, nav, LanguageSelector (F002), bell/account hoặc login link | A1, A4, A5 | `components/home/home-header.tsx` (planned) |
| `CountdownTimer` | Hiển thị 3 ô số + nhãn, dùng `useCountdown` | A2 | `components/home/countdown-timer.tsx` (planned) |
| `AwardCard` | Thẻ giải thưởng (ảnh+tiêu đề+mô tả+Chi tiết) | A1 | `components/home/award-card.tsx` (planned) |
| `NotificationBell` | Nút chuông + panel dialog rỗng | A5 | `components/home/notification-bell.tsx` (planned) |
| `AccountMenu` | Nút tài khoản + menu Hồ sơ/Đăng xuất/Trang quản trị | A4 | `components/home/account-menu.tsx` (planned) |
| `WidgetButton` | Nút nổi góc dưới phải + menu 2 mục | A6 | `components/home/widget-button.tsx` (planned) |
| `HomeFooter` | Footer: logo+4 link+bản quyền | A1 | `components/home/home-footer.tsx` (planned) |
| `getCurrentAccount` | Đọc session+role phía server, fail-open member | A1 | `lib/auth/get-current-user-role.ts` (planned) |
| `useCountdown` | Hook tick mỗi phút từ prop server-seeded | A2 | `hooks/use-countdown.ts` (planned) |
...
**Source:** `proxy.ts` (planned) · `app/page.tsx` (planned, thay thế `redirect()` cũ)
...
**Source:** `lib/event/countdown.ts` (planned, `parseTargetDate`) · `app/page.tsx` (planned)
```text
...
**Linked FR:** FR-202
**Used in:** A2
**Source:** `lib/event/countdown.ts` (planned)
...
**Used in:** A1
**Source:** `lib/auth/get-current-user-role.ts` (planned)
**Type:** api-call
...
- *(A2)* Chấp nhận lệch ≤1 phút giữa SSR render và client hydrate (`suppressHydrationWarning` trên 3 ô số) — không coi là bug.
...
|---|---|---|---|---|
| — | 1 | `HomeCopy` | `components/home/home-copy.ts` (planned) | Nguồn nội dung tĩnh mặc định (vi) cho toàn trang chủ |
| A1 | 2 | `HomePage` | `app/page.tsx` (planned) | Server Component, entry point `/`, thay thế `redirect()` cũ |
| A1 | 3 | `getCurrentAccount` | `lib/auth/get-current-user-role.ts` (planned) | Đọc session + role, fail-open `member` |
| A2 | 4 | `useCountdown`, `remaining`, `parseTargetDate` | `hooks/use-countdown.ts`, `lib/event/countdown.ts` (planned) | Tính + tick đếm ngược |
| A3 | 5 | `NavLink` | `components/home/nav-link.tsx` (planned) | Active-link scroll-to-top |
...
```text
GET / -> HomePage [đọc cookie session] -> getCurrentAccount(supabase) -> {email, role}
  -> props xuống HomeHeader/CountdownTimer/AccountMenu -> render tĩnh + targetIso/initialNowMs
client tick: useCountdown(targetIso, initialNowMs) -> setInterval 60s -> remaining() -> re-render 3 ô số
```
...
| Feature List | [feature-list.md](../../generated/feature-list.md) | F003 (provisional — cấp thật lúc promote) | [ ] |
| Entities | [entities.md](../../generated/entities.md) | MODEL001 (tái dùng từ F002); HomeCopy/UserRole: TBD (draft) | [ ] |
| Screens | [functional-spec.md § 6](./functional-spec.md#6-screens) | SCR-home (draft) | [ ] |
