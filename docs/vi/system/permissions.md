...
**Generated**: 2026-09-06 (cập nhật sau khi Homepage SAA `/` lên code thật, F003_Homepage — xem `plans/260906-0042-homepage-saa-page/clarifications.md`)
**Analysis Scope**: `/` (nay PUBLIC, không còn route-guard), `/login`, `/todo`, `/auth/callback` — toàn bộ authorization surface của app. 5 route Homepage liên kết tới (`/awards`, `/kudos`, `/standards`, `/profile`, `/admin`) CHƯA tồn tại, ngoài phạm vi phân tích quyền vì chưa có code.
