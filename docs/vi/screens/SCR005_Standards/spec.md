...
| E05 | Lưới 6 badge (ảnh + caption text) | card (group) | — | — | Always | — | static | raw (caption uppercase, giữ "ROOT FURTHER") | — | binding: `SecretBoxBadge` × 6, note "repetitive groups collapse to one row" |
...
| E08 | Nút "Đóng" (icon X + text) | button | — | Enabled | Always | Click → `router.back()`, fallback `ROUTES.HOME` | computed | raw | — | binding: `useStandardsClose().handleClose`, wired qua `StandardsClient` |
...
|----------------|-----------|------------------------|--------|
| `router.back()` hay fallback `/` khi click "Đóng" | `window.navigation?.canGoBack` (Navigation API — thay cho `window.history.length` đề xuất ở spec draft, đã đổi khi implement) | `true`: `router.back()`; `false`/`undefined`: `router.push(ROUTES.HOME)` | `_hooks/use-standards-close.ts` |
...
| Aspect | Status | Notes |
|--------|--------|-------|
| ARIA roles/labels | [IMPLEMENTED] | `<h1>Thể lệ</h1>`, 3 `<h2>` cấp section; `HeroBadgeTierRow` dùng `alt={tên tier}` (New Hero/...); `SecretBoxBadge` dùng `alt="" aria-hidden="true"` (caption là `<p>` text riêng) |
| Keyboard navigation | [IMPLEMENTED] | Nút "Đóng" là `<button>` chuẩn (Tab/Enter/Space hoạt động qua trình duyệt); "Viết KUDOS" là `<a href>` chuẩn |
| Focus management | [IMPLEMENTED] | `focus-visible:ring-2 focus-visible:ring-login-button focus-visible:outline-none` trên cả 2 nút footer (`standards-footer-actions.tsx:37,51`) |
...
| Screen reader compatibility | [IMPLEMENTED] | Caption 6 badge là `<p>` text thật trong DOM (không alt-text thay thế) — `secret-box-badge.tsx:34-36` xác nhận đúng FR-103 |
