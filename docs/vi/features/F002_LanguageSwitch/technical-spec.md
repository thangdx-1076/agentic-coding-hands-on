...
**Cập nhật kiến trúc (không có trong bản gốc):** `LanguageSelector` đã được promote thành component
dùng chung ở `src/app/_components/language-selector/` (trước đây nằm riêng dưới `components/login/`)
...
- **BR-001 — Mọi giá trị locale từ client luôn được chuẩn hoá qua whitelist {vi, en} trước khi
  ...
  `isSupportedLocale` (`src/lib/i18n/locale.ts:34-39`) — giá trị không khớp đúng `"vi"`/`"en"`
...
- **BR-002 — Đổi ngôn ngữ dùng CHUNG một `useTransition` với hành động đăng nhập Google, nên
  ...
  `src/app/(public)/login/_hooks/use-login-actions.ts:40,60-66` — đây là hành vi cố ý được giữ nguyên qua lần refactor
...
- Không có `redirect()` — round-trip của Server Action tự re-render cây trang với output mới của
  ...
  không cần tải lại trang (`src/app/_actions/set-locale.ts:20-23` comment nguồn).
- Nếu `cookies().set()` bị gọi ngoài ngữ cảnh Server Action, hàm ném `Error` mô tả rõ nguyên nhân
  thay vì âm thầm không làm gì (`src/app/_actions/set-locale.ts:35-43`) — xem § 5.3 Unresolved Questions #1.
...
|---|---|---|
| A1 | Cookie `NEXT_LOCALE` mang giá trị rác/không hợp lệ (vd. `"fr"`, `"vi; en"`, `"EN"` sai hoa/thường) ở request kế tiếp | `normalizeLocale` (`src/lib/i18n/locale.ts:49-51`) âm thầm rơi về `DEFAULT_LOCALE = "vi"`; không có lỗi hiển thị, không có log — xem `src/lib/i18n/locale.test.ts` (test đã confirm 4/4 case rác này) |
| A1 | Lần đầu ghé thăm, chưa từng có cookie `NEXT_LOCALE` | `normalizeLocale(undefined)` trả về `"vi"` ngay từ request đầu tiên (`src/i18n/request.ts:24`) — không có nhấp nháy nội dung thiếu |
...
| A1-A2 | Bất kỳ lỗi bất đồng bộ nào trong `startTransition` (bao gồm `setLocale` ném `Error` — § 5.3 #1) | `[INFERRED]` không có `try/catch` nào ở `handleSelectLocale` (`src/app/(public)/login/_hooks/use-login-actions.ts:60-66`) hay ở caller — lỗi thoát ra ngoài transition, khả năng cao bị React coi là uncaught exception; chưa xác nhận được hành vi thật vì lỗi chỉ xảy ra khi gọi ngoài ngữ cảnh Server Action, không tái hiện được trong test |
...
|---|---|---|---|
| `LanguageSelector` | Trình bày thuần: nút trigger + menu ARIA menu-button, class/a11y attribute — nay là component dùng chung ở root, không còn riêng của `/login` | A1, A2 | `src/app/_components/language-selector/language-selector.tsx` |
...
`src/utils/a11y/roving-index.test.ts`, coverage 100% (không React/DOM nên chạy được ở môi trường
`node` của vitest).
...
- **SC-005** *(A2)* Giá trị locale không hợp lệ luôn được chuẩn hoá về `vi` (covers FR-001,
  ...
  yêu cầu bảo mật whitelist FR-601, xem `src/lib/i18n/locale.ts:49-51`) — unit test
  `src/lib/i18n/locale.test.ts` (8 case, bao gồm rác/injection/case-mismatch)
...
- **SC-007** *(A1)* Khi cookie `NEXT_LOCALE` đã lưu sẵn một giá trị rác/không hợp lệ (vd. `"fr"`)
  từ trước, request kế tiếp đi qua path ĐỌC cookie (`src/i18n/request.ts:22-41`) phải âm thầm fallback
  ...
  rồi assert hành vi của `getRequestConfig` (`src/i18n/request.ts:22-41`) khi ĐỌC lại;
  `src/lib/i18n/locale.test.ts` chỉ kiểm `normalizeLocale` ở cấp đơn vị, không đi qua
...
- *(A2)* `setLocale` được giả định LUÔN được gọi đúng bên trong ngữ cảnh Server Action (qua
  ...
  `src/app/_actions/set-locale.ts:35-43` được ghi nhận là code đã đọc, chưa xác nhận runtime thật.
- *(A1)* Giả định `itemCount` (2, từ `OPTIONS.length`) không đổi trong vòng đời component — đúng
  như comment nguồn ở `src/hooks/use-menu-keyboard-nav.ts:59-62` (giới hạn hiện tại, không phải bug).
- *(A2)* Giả định không có cơ chế debounce/lock nào giữa các lần chọn ngôn ngữ liên tiếp — suy ra
  từ việc đọc `handleSelectLocale` (`src/app/(public)/login/_hooks/use-login-actions.ts:60-66`), không kiểm chứng bằng
  test race-condition thật.

### 5.3 Unresolved Questions

1. **Hành vi khi `setLocale` ném lỗi** *(A2)*: `cookies().set()` ném ngoài ngữ cảnh Server Action
   (`src/app/_actions/set-locale.ts:35-43`) nhưng `handleSelectLocale` không có `try/catch`
   (`src/app/(public)/login/_hooks/use-login-actions.ts:60-66`) — chưa xác nhận được React/Next.js xử lý lỗi này ra sao
...
|---|---|---|---|---|
| A1 | 1 | `AppLocale`, `normalizeLocale` | `src/lib/i18n/locale.ts:1-51` | Union type + choke-point chuẩn hoá locale, dùng bởi cả A1 và A2 |
| A1 | 2 | `LanguageSelector` | `src/app/_components/language-selector/language-selector.tsx:1-102` | Trigger + menu, trình bày thuần |
| A1 | 3 | `useMenuKeyboardNav` | `src/hooks/use-menu-keyboard-nav.ts:1-177` | State open/close, roving-focus, mọi keydown handler |
