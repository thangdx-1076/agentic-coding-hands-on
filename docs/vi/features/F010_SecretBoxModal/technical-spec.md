...
| `SecretBoxDialog` | Component thuần trình bày — nhận `copy`/`state`/`unopenedCount`/`awardedBadgeKey`/`canOpen`/`busy` + callback làm props, render 2 trạng thái (`unopened`/`revealed`) | A2, A3 | `src/app/(public)/kudos/_components/secret-box-dialog.tsx` |
...
| `openSecretBox` / `toSecretBoxClient` (DAL) | Gọi `.rpc("open_secret_box")` qua client thu hẹp, boundary-check `unknown` trước khi tin, map 2 lỗi RPC đã biết (`no_boxes_left`/`unauthenticated`), mọi lỗi khác throw | A3 | `src/dal/secret-box.ts`, `src/dal/secret-box-client.ts` |
...
| `secretBoxBadgeAsset` / `isBadgeKey` | Tra asset badge 64×64 theo `BadgeKey`; khai báo LẠI 6 giá trị badge cục bộ thay vì tái dùng bảng của `/standards`, để tránh sideways import giữa 2 route con cùng cấp (xem § 5.2) | A3 | `src/app/(public)/kudos/_utils/secret-box-badge-asset.ts` |
...
- *(A3)* **Giả định ban đầu SAI, đã sửa theo code thật:** bản draft giả định phải DỜI
  `SECRET_BOX_BADGES`/`SECRET_BOX_BADGE_SIZE` từ `standards-copy.ts` sang
  `src/app/_shared/secret-box-copy.ts`. Code thật không dời file đó — thay vào đó
  `src/app/(public)/kudos/_utils/secret-box-badge-asset.ts` khai báo LẠI 6 giá trị badge cục bộ
  (đúng lý do "tránh sideways import" mà giả định này đã nêu, chỉ khác giải pháp: nhân đôi hằng số
  thay vì di chuyển file dùng chung). `standards-copy.ts` không đổi.
- *(A1)* Giả định tooltip `title` hiện có trên nút disabled ở `kudos-stat-list.tsx` đã đủ giải
  thích lý do, không cần đổi copy.

### 5.3 Unresolved Questions

Cả 2 câu hỏi dưới đây đã RESOLVED — xác nhận trực tiếp trên code/file thật sau khi merge, giữ lại
để lộ dấu vết đã từng là câu hỏi mở:

1. **Copy song ngữ** *(A3)* — RESOLVED: `messages/en.json` có đủ 6 khoá
   `standards.secretBoxSection.badges.*`, khớp `vi.json` (`beyondTheBoundary`, `flowToHorizon`,
   `revival`, `rootFurther`, `stayGold`, `touchOfLight`).
2. **Hình dạng RPC thật** *(A3)* — RESOLVED: `0011_secret_box.sql`'s `open_secret_box()` khai báo
   `RETURNS TABLE (badge_key text, unopened int)`, đúng field name sketch đã giả định;
   `src/dal/secret-box.ts`'s `parseOpeningRow` boundary-check đúng 2 field này qua `unknown`.

### 5.4 Source References

Source code đã viết và merge (`status: implemented`). File thật đã chạm tới, đã đọc và xác nhận:
`supabase/migrations/0011_secret_box.sql`, `src/dal/secret-box.ts` (+ test),
`src/dal/secret-box-client.ts` (+ test), `src/dal/kudos-stats.ts` (mở rộng),
`src/app/(public)/kudos/page.tsx`,
`src/app/(public)/kudos/_components/secret-box-launcher.tsx`,
`src/app/(public)/kudos/_components/secret-box-dialog.tsx` (+ story) — KHÔNG phải
`secret-box-modal.tsx` như bản draft ghi,
`src/app/(public)/kudos/_hooks/use-secret-box-dialog.ts` (+ test),
`src/app/(public)/kudos/_actions/open-secret-box.ts` (+ test),
`src/app/(public)/kudos/_utils/secret-box-badge-asset.ts` (+ test) — file mới, KHÔNG phải
`src/app/_shared/secret-box-copy.ts` như bản draft giả định (xem § 5.2),
`src/app/(public)/kudos/_shared/build-kudos-copy.ts` (mở rộng, thêm khoá `secretBox.*`),
`messages/vi.json` + `messages/en.json`, `tests/e2e/secret-box.spec.ts`,
`tests/e2e/kudos.spec.ts` (dòng 743-748).
...
| Feature List | [feature-list.md](../../docs/generated/feature-list.md) | F010 | [ ] |
