...
**Kết quả Delivery (2026-09-07):** GREEN — `pnpm test:e2e tests/e2e/profile.spec.ts --reporter=list`
exit 0, **22/22 pass**, 0 skip (contract `[C1]`-`[C18]`, § 4.5). RED trước code đã ghi tại
`plans/260907-1224-profile-page/evidence/red-evidence.md` (404 vì route chưa tồn tại); GREEN cuối
tại `evidence/green-evidence.md`. Regression 4 spec e2e cũ (home/awards/standards/login) không
giảm số test. CI chỉ chạy `[C17]` — 21 test `@auth` còn lại cần Supabase local, xem functional-spec.md
§ 14 cho chi tiết đầy đủ và giới hạn CI.
